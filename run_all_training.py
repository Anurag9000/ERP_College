#!/usr/bin/env python3
"""One-command ERP_College training-surface authority.

ERP_College is currently a Java/Maven + frontend application with no retained ML
optimizer/training surface.  The root does not manufacture fake training jobs: it
runs a source/build-manifest certificate that fails closed if any supported ML
framework dependency, estimator fit primitive, neural trainer or optimizer surface
is later introduced.  Such a future surface must then be explicitly wired into the
same OPF-derived controller before the repository can pass again.

The shared controller remains canonical v37 and resource/process scheduling remains
exclusively in the literal byte-pinned OPF_ADP runtime.
"""
...
PROFILE = {
    ...
    "scientific_authority": "training_control/erp_college_no_training_authority_v1.py",
    "jobs": [
        {
            ...
            "early_stopping_exception_reason": "source/build-manifest classification audit",
    ...
    "dynamic_registry_covers": [
        ...
        "pom.xml",
        "package.json",
        "training_control/erp_college_no_training_authority_v1.py",
    ],
    "ignore_entrypoints": [
        "run_all_training.py",
...
def main() -> int:
    ...
    profile_path = ROOT / ".training_control" / "erp_college_no_training_v1_v37.json"
    _atomic(
        profile_path,
