...
public class ReportsPanel extends JPanel {
    ...
    private JPanel createEnrollmentTrendsPanel() {
        ...
        statsPanel.add(createStatCard("Total Sections", String.valueOf(totalSections), PastelTheme.PASTEL_BLUE_DARK));
        statsPanel.add(createStatCard("Total Enrolled", String.valueOf(totalEnrolled), PastelTheme.PASTEL_GREEN_DARK));
        statsPanel.add(createStatCard("Total Capacity", String.valueOf(totalCapacity), PastelTheme.PASTEL_PURPLE_DARK));
    ...
    private JPanel createWaitlistPressurePanel() {
        ...
        for (Section s : sections) {
            int waitlist = s.getWaitlistedStudentIds().size();
            int capacity = s.getCapacity();
            String pressure = waitlist > capacity * 0.5 ? "HIGH" : waitlist > capacity * 0.2 ? "MEDIUM" : "LOW";
    ...
    private JPanel createFinancialArrearsPanel() {
        ...
        summaryPanel.add(createStatCard("Collection Rate", String.format("%.1f%%",
                students.stream()
                        .mapToDouble(s -> s.getTotalFees() > 0 ? (s.getFeesPaid() / s.getTotalFees() * 100) : 0)
                        .average().orElse(0)),
        ...
        students.stream()
                ...
                .forEach(s -> {
                    model.addRow(new Object[] {
                            ...
                            s.getNextFeeDueDate() != null ? s.getNextFeeDueDate().toString() : "N/A"
                    });
                });
    ...
    private void exportEnrollmentTrends() {
        JOptionPane.showMessageDialog(this, "Enrollment trends exported to enrollment_report.csv");
